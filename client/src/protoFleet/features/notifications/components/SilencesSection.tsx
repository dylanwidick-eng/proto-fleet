...
const SilencesSection = () => {
  ...
  const sortedSilences = useMemo<SilenceWithActive[]>(() => {
    const now = Date.now();
    const withActive: SilenceWithActive[] = silences.map((s) => {
    ...
    return withActive.sort(
      (a, b) =>
        Number(b.active) - Number(a.active) || a.starts_at.localeCompare(b.starts_at),
    );
  }, [silences]);

  const ruleNameById = useCallback(
    (id: string) => rules.find((r) => r.id === id)?.name ?? id,
    [rules],
  );

  const formatTarget = useCallback(
    (silence: SilenceWithActive): string => {
      if (silence.scope.kind === "rule")
        return silence.scope.rule_id ? ruleNameById(silence.scope.rule_id) : "—";
  ...
  const handleLift = useCallback(
    (silence: SilenceWithActive) => {
      removeSilence(silence.id);
      ...
      target: {
        component: (silence) => (
          <span className="flex items-center gap-2">
            ...
            {silence.active ? (
              <span className="rounded bg-state-success-fill/10 px-2 py-0.5 text-200 text-state-success-fill">
                Active
              </span>
            ) : (
