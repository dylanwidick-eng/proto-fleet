...
const ActivityRow = ({
  ...
  const Icon: (props: IconProps) => ReactNode = isFailed ? Alert : getActivityIcon(entry.eventType);
  return (
    <FeedRowShell testId="list-row" isRead={isRead} onClick={() => onSelect(entry)}>
      <div className="flex items-start gap-2">
        ...
        {isFailed ? (
          <span className={clsx("shrink-0 text-200", !isRead && "text-intent-critical")}>Failed</span>
        ) : null}
      </div>
