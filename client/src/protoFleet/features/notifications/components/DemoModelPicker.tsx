...
const DemoModelPicker = () => {
  ...
  const [enabled, setEnabled] = useState(() => readDemoFlag());

  useEffect(() => {
    setEnabled(readDemoFlag());
  }, []);
      ...
      {open ? (
        <div className="flex w-72 flex-col gap-1 rounded-2xl border border-surface-10 bg-surface-elevated-base p-3 shadow-300">
          <div className="flex items-center justify-between pb-1">
            <span className="text-100 font-medium uppercase tracking-wider text-text-primary-50">
              Notification model
            </span>
            <button
              type="button"
            ...
            >
              <span
                className={clsx(
                  ...
                  m.id === current ? "text-text-primary" : "text-text-primary",
                )}
              >
                {m.label}
              </span>
