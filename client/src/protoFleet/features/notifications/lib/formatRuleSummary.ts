...
// Example: "Applies to 4 devices" / "All devices" / "Group grp_dalton_bottom".
export const formatRuleScopeSummary = (rule: Rule): string => {
  ...
  if (kind === "devices") {
    ...
    return `Applies to ${count} ${count === 1 ? "device" : "devices"}`;
  }
...
// Last-fired subtext for the Condition cell, mirroring the Schedules "next run" line.
export const formatRuleLastFired = (rule: Rule): string => {
  if (!rule.last_fired_at) return "Never fired";
  const d = new Date(rule.last_fired_at);
  const date = d.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" });
  const time = d.toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" });
  return `Last fired ${date}, ${time}`;
};
